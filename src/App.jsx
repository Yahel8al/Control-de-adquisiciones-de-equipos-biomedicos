...
const TIPO_STYLE = {
  "Nuevo":      { bg: "#DCF5EF", text: "#0B6E64", dot: "#12A594" },
  "Reposición": { bg: "#FDF3D8", text: "#8A6D00", dot: "#E3A008" },
};
const CRITICIDAD_STYLE = {
  "Crítica": { bg:"#FBE2DE", text:"#A3341E", dot:"#E4572E" },
  "Alta": { bg:"#FBE2DE", text:"#A3341E", dot:"#E4572E" },
  "Media": { bg:"#FDF3D8", text:"#8A6D00", dot:"#E3A008" },
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.25fr .9fr",gap:16}}>
              ...
              <div className="card" style={{padding:"16px 18px"}}><div className="disp" style={{fontSize:13.5,fontWeight:700}}>Tipo de equipo:</div><div style={{fontSize:11,color:SLATE_LIGHT}}>Comparación por tipo de adquisición</div><ResponsiveContainer width="100%" height={250}><BarChart data={byTipo} margin={{top:15,right:18,left:4}}><CartesianGrid strokeDasharray="3 3" stroke={BORDER} vertical={false}/><XAxis dataKey="tipo" tick={{fontSize:11,fill:SLATE}}/><YAxis tickFormatter={fmtUSDk} tick={{fontSize:11,fill:SLATE_LIGHT}}/><Tooltip content={(props)=>{const p=props?.payload?.[0]?.payload;if(!p)return null;return <div style={{background:"#fff",border:`1px solid ${BORDER}`,borderRadius:8,padding:"9px 11px",fontSize:12,boxShadow:"0 4px 14px rgba(11,42,61,.12)"}}><div style={{fontWeight:700,color:INK,marginBottom:5}}>{p.tipo}</div><div>Solicitudes: <b>{p.n}</b> - <b>{p.unidades}</b> equipos</div><div>Inversión: <b>{fmtUSD(p.inversion)}</b></div></div>}}/><Bar dataKey="inversion" radius={[5,5,0,0]}>{byTipo.map((e,i)=><Cell key={i} fill={TIPO_STYLE[e.tipo].dot}/>)}</Bar></BarChart></ResponsiveContainer></div>
              ...
              <div className="card" style={{padding:"16px 18px"}}><div className="disp" style={{fontSize:13.5,fontWeight:700}}>Prioridad</div><div style={{fontSize:11,color:SLATE_LIGHT}}>Alta, Media y Baja</div><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={PRIORIDADES.map(pr=>{const rows=data.filter(r=>r.prioridad===pr);return {prioridad:pr,n:rows.length,unidades:rows.reduce((s,r)=>s+Number(r.cantidad||0),0),inversion:rows.reduce((s,r)=>s+Number(r.total||0),0)};}).filter(x=>x.n>0)} dataKey="n" nameKey="prioridad" cx="50%" cy="47%" innerRadius={48} outerRadius={86} paddingAngle={2}>{PRIORIDADES.map(pr=><Cell key={pr} fill={PRIORITY_STYLE[pr].text}/>)}</Pie><Tooltip content={(props)=>{const p=props?.payload?.[0]?.payload;if(!p)return null;return <div style={{background:"#fff",border:`1px solid ${BORDER}`,borderRadius:8,padding:"9px 11px",fontSize:12}}><div style={{fontWeight:700,color:INK,marginBottom:5}}>{p.prioridad}</div><div>Solicitudes: <b>{p.n}</b> - <b>{p.unidades}</b> equipos</div><div>Inversión: <b>{fmtUSD(p.inversion)}</b></div></div>}}/><Legend iconType="circle" wrapperStyle={{fontSize:11}}/></PieChart></ResponsiveContainer></div>
               <div className="card" style={{padding:"16px 18px"}}><div className="disp" style={{fontSize:13.5,fontWeight:700}}>Presupuesto vs. ejecutado</div><ResponsiveContainer width="100%" height={250}><BarChart data={budgetByArea.filter(x=>x.presupuesto>0||x.ejecutado>0)} margin={{top:10}}><CartesianGrid strokeDasharray="3 3" stroke={BORDER} vertical={false}/><XAxis dataKey="area" tick={{fontSize:9.5,fill:SLATE_LIGHT}} angle={-30} textAnchor="end" height={70} interval={0}/><YAxis tickFormatter={fmtUSDk} tick={{fontSize:11,fill:SLATE_LIGHT}}/><Tooltip formatter={(v,n)=>[fmtUSD(v),n]} contentStyle={{fontSize:12,borderRadius:8}}/><Legend wrapperStyle={{fontSize:11}}/><Bar dataKey="presupuesto" name="Presupuesto" fill={SLATE_LIGHT} radius={[3,3,0,0]}/><Bar dataKey="ejecutado" name="Ejecutado" fill={TEAL} radius={[3,3,0,0]}/></BarChart></ResponsiveContainer></div>
            ...
            <div className="card" style={{ padding: "16px 18px", marginBottom: 16 }}>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={budgetByArea} margin={{ top: 10 }}>
                  ...
                  <Bar dataKey="presupuesto" name="Presupuesto" fill={SLATE_LIGHT} radius={[3, 3, 0, 0]} />
